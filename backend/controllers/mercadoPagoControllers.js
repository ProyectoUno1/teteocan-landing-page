...
const webhookSuscripcion = async (req, res) => {
  try {
    ...
    if (topic === 'preapproval' && mpNotification.data?.id) {
      const preapproval_id = mpNotification.data.id;

      // Consultar el preapproval real desde Mercado Pago
      const preapprovalResponse = await fetch(`https://api.mercadopago.com/preapproval/${preapproval_id}`, {
        headers: {
          Authorization: `Bearer ${process.env.MP_ACCESS_TOKEN}`,
          'Content-Type': 'application/json'
        }
      ...
      if (preapprovalInfo.status !== 'authorized') {
        console.log('Suscripción aún no autorizada:', preapprovalInfo.status);
        return res.status(200).send('Suscripción aún no autorizada.');
      }
      ...
      const reqMock = { body: orden?.orderData || {
        nombrePaquete: preapprovalInfo.reason || 'Paquete sin nombre',
        resumenServicios: 'Servicios no especificados',
        monto: preapprovalInfo.auto_recurring?.transaction_amount || 0,
        fecha: new Date().toLocaleDateString('es-MX'),
        clienteEmail: preapprovalInfo.payer_email || 'cliente@example.com',
      ...
      if (orden) delete ordenesPendientes[preapproval_id];
      console.log('Webhook procesado: correos enviados.');
